...
在上面的例子中,每个文本字符对应到当个字节, 但是, 情况不总是这样, 比如一个Unicode雪人字符[清单 8-3](#listing-8-3), 虽然不常用, 但在JavaScript确是一个有效的字符串数据. [清单 8-4](#listing-8-4) 显示了此字符的二进制标识. 注意UTF-8编码要求3个字节来表示这个字符.
